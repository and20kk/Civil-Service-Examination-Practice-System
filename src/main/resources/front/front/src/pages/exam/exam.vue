<template>
	<div>
		<div class="top-content">
			...
			<div class="top-content-center" v-cloak>距离练习结束还有 <el-tag type="danger">{{this.hour}}</el-tag> 时 <el-tag type="danger">{{this.minute}}</el-tag> 分 <el-tag type="danger">{{this.second}}</el-tag> 秒</div>
			<div class="top-content-right">
				<el-button type="danger" @click="doneExam(0)">结束练习</el-button>
			</div>
		</div>
		<div class="question-content" v-for="(item, index) in questionList" :key="item.id" v-if="curQuesIndex == index">
			<div class="question-title" v-if="!end">
				题目 ({{item.score}}分)：<span class="ql-snow ql-editor" v-html="item.questionname"></span>
			...
			<div class="question-answer">
				<div v-if="!isSubmit && !end">
					...
					<el-input style="width: 400px;" v-if="item.type == 4" type="textarea" v-model="examrecord.myanswer" placeholder="请输入答案"></el-input>
				</div>
				<div v-if="isSubmit && !end">
					<div class="question-title">填写答案： {{question.type == 1 ? myanswer.join(',') : examrecord.myanswer}}</div>
					<div v-if="question.answer == examrecord.myanswer&&question.type!=4" class="question-title"><el-tag type="success">正确</el-tag> 解析：{{question.answer}}, {{question.analysis}}</div>
					<div v-if="question.answer != examrecord.myanswer&&question.type!=4" class="question-title"><el-tag type="danger">错误</el-tag> 解析：{{question.answer}}, {{question.analysis}}</div>
				</div>
				<div v-if="end">
					<div class="question-title">练习成绩：<el-tag type="danger">{{totalScore}}</el-tag> 分</div>
				</div>
			</div>
			<div class="question-btn">
				<el-button v-if="!isSubmit && !end" type="primary" @click="submitAnswer">提交答案</el-button>
				<el-button v-if="isSubmit && !end" type="success" @click="nextQuestion">下一题</el-button>
				<el-button v-if="end" type="success" @click="doneExam(1)">结束练习</el-button>
			</div>
		</div>
	</div>
</template>
    ...
    //数据集合
		...
		methods: {
			...
			submitAnswer() {
				...
				this.examrecord.myscore = this.question.type!=4?(this.examrecord.myanswer == this.question.answer ? this.question.score : 0):0
				this.examrecord.options = this.question.options;
				this.examrecord.type = this.question.type;
				this.examrecord.ismark = this.hassubject? 0 : 1
				this.examrecord.username = this.user.yonghuxingming;
			...
			doneExam(type) {
				if (type == 0) {
					this.$confirm('确定离开练习?未答题目按0分计', '提示', {
