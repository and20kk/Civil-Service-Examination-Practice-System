	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('tixingxinxi','新增')" type="primary" @click="add('/index/tixingxinxiAdd')">
			...
			<div class="list">
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)">
						<div class="imgbox">
							<img @click.stop="imgPreView(item.tixingfengmian)" v-if="item.tixingfengmian && item.tixingfengmian.substr(0,4)=='http'&&item.tixingfengmian.split(',w').length>1" :src="item.tixingfengmian" class="image" />
							<img @click.stop="imgPreView(item.tixingfengmian.split(',')[0])" v-else-if="item.tixingfengmian && item.tixingfengmian.substr(0,4)=='http'" :src="item.tixingfengmian.split(',')[0]" class="image" />
							<img @click.stop="imgPreView(baseUrl + (item.tixingfengmian?item.tixingfengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.tixingfengmian?item.tixingfengmian.split(',')[0]:'')" class="image" />
						</div>
						<div class="infoBox">
							<div class="name">{{item.tixingbiaoti}}</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`tixingxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/tixingxinxiDetail', query: params});
			},
